document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchAvailability() {
    ...
    fetch(`/customer/field/${fieldId}/check-availability?date=${selectedDate}`)
      ...
      .catch((error) => {
        ...
        timeSlotsContainer.innerHTML =
          '<div class="alert alert-danger">Gagal memuat ketersediaan jadwal. Silakan coba lagi.</div>';
      });
  }
  ...
  function renderTimeSlots(data) {
    ...
    if (!data.operating_hours) {
      timeSlotsContainer.innerHTML =
        '<div class="alert alert-warning">Informasi jam operasional tidak tersedia.</div>';
      return;
    }
